-module(opmake_handler).
-behaviour(gen_event).

-record(state, {access_token :: opmake:access_token(),
                report_handlers :: [opmake_handlers:handler()],
                items = [] :: [term()]|[],
                requests = [] :: [{reference(), non_neg_integer()}],
                batch_max :: pos_integer()|undefined,
                time_max :: opmake:ms()|undefined,
...
init([AccessToken, Opts]) ->
    ...
    ok = hackney_pool:start_pool(opmake, [{max_connections, ?MAX_CONN}]),
    {ok, #state{access_token=AccessToken,
                ...
                details=opmake_encoder:create(Opts)
               }}.
...
send_items(#state{items=Items, requests=Requests, endpoint=Endpoint,
    ...
    Message = opmake_encoder:encode(Items, AccessToken, Details),
...
info(Details) ->
    error_logger:info_report(opmake_report, Details).
